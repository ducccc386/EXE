/**
 * mocks/tutors.mock.js
 * ─────────────────────────────────────────────────────────────────────────────
 * [MOCK] Dữ liệu gia sư giả dùng khi CHƯA có backend.
 *
 * ⚠️  XÓA FILE NÀY khi các endpoint sau đã hoạt động:
 *   GET /api/tutors            → danh sách gia sư (có filter params)
 *   GET /api/tutors/:id        → chi tiết 1 gia sư
 *
 * Các file đang import từ đây:
 *   - pages/tutor/TutorListingPage.jsx  (dùng mock qua getTutors() fallback)
 *   - pages/tutor/TutorProfilePage.jsx  (TODO: đổi sang getTutorById() từ tutorApi)
 *
 * Shape mỗi tutor object (để backend biết cần trả về gì):
 * {
 *   id, name, initials, avatarBg, coverGradient, coverImage,
 *   title, university, subject, location,
 *   rating, reviewCount,
 *   pricePerHour, priceMin, priceMax, rateType ("hour"|"session"),
 *   bio, tags[], verified,
 *   reviews[]: { name, rating, text, date },
 *   certificates[]: { name, year, verified }
 * }
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
/** Lấy tutor theo id — dùng khi chưa có GET /api/tutors/:id */
export function getTutorByIdMock(id) {
