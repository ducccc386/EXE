...
function Avatar({ name, size = "md", online }) {
  ...
  return (
    <div className="relative flex-shrink-0">
      <div className={`${sizeClass} rounded-full bg-gradient-to-br ${colors[colorIdx]} flex items-center justify-center text-white font-bold shadow-sm`}>
        {initials}
      </div>
      {online !== undefined && (
...
function FriendItem({ friend, active, onClick, t }) {
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-150 text-left group ${active ? "bg-blue-50 border border-blue-100 shadow-sm" : "hover:bg-gray-50 border border-transparent"
        }`}
    >
...
function MessageBubble({ msg, isMe }) {
  return (
    <div className={`flex items-end gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
      <div className={`max-w-[72%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm ${isMe ? "bg-blue-500 text-white rounded-br-md" : "bg-white text-gray-800 border border-gray-100 rounded-bl-md"}`}>
        <p>{msg.content}</p>
        <span className={`block text-[10px] mt-1 ${isMe ? "text-blue-200 text-right" : "text-gray-400"}`}>
          {msg.time}
        </span>
      </div>
    </div>
    ...
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-8">
      <div className="w-20 h-20 rounded-full bg-blue-50 flex items-center justify-center">
        <svg className="w-10 h-10 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.77 9.77 0 01-4-.8l-4 1 1-4A8.96 8.96 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
        </svg>
      </div>
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (selectedFriend.unread > 0) {
      ...
      const lastMsgId = msgs[msgs.length - 1]?.id || selectedFriend.id;
      markAsRead(lastMsgId).catch(() => { });
    }
  ...
  const handleSend = useCallback(async () => {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      try {
        ...
        // Silent fail — optimistic UI update
      } finally {
        setSending(false);
      }
    }
