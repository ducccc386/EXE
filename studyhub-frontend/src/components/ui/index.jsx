 ...
 * Shared UI primitives dùng toàn app.
 */

// ─── VerifiedBadge ────────────────────────────────────────────────────────────
export function VerifiedBadge({ size = "sm" }) {
  const sz = size === "sm" ? "text-xs px-2 py-0.5 gap-1" : "text-sm px-3 py-1 gap-1.5";
  const ic = size === "sm" ? "w-3 h-3" : "w-4 h-4";
  return (
    <span className={`inline-flex items-center font-bold rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200 ${sz}`}>
      <svg className={ic} fill="currentColor" viewBox="0 0 20 20">
...
export function StarRating({ rating, count, size = "sm" }) {
  const starSz = size === "sm" ? "w-3.5 h-3.5" : "w-5 h-5";
  return (
    <div className="flex items-center gap-1">
      <div className="flex items-center gap-0.5">
        {[1, 2, 3, 4, 5].map((s) => (
          <svg key={s} className={`${starSz} ${s <= Math.round(rating) ? "text-amber-400" : "text-gray-200"}`} fill="currentColor" viewBox="0 0 20 20">
...
// ─── Avatar ───────────────────────────────────────────────────────────────────
export function Avatar({ initials, avatarUrl, bg = "bg-blue-500", size = "md" }) {
  const sizes = { sm: "w-9 h-9 text-sm", md: "w-12 h-12 text-base", lg: "w-20 h-20 text-2xl", xl: "w-28 h-28 text-3xl" };
  if (avatarUrl) {
    return <img src={avatarUrl} alt={initials} className={`${sizes[size]} rounded-full object-cover flex-shrink-0`} />;
  }
  return (
...
// ─── formatPrice (re-exported từ utils/format.js để backward compat) ──────────
